...
| Customer FE — C. Auth & Account | `apps/web` | 🔶 In progress — split into C1 Core Auth → C2 Account profile → C3 Google OAuth → C4 2FA TOTP | see C1 below |
...
| Customer FE — C2. Account profile | `apps/web` | ⬜ Not started (`/account`; `GET/PATCH /users/me`) | TBD |
| Customer FE — C3. Google OAuth | `apps/web` | ⬜ Not started (wire `signInWithOAuth` into the existing `/auth/callback` + GoogleButton seam) | TBD |
| Customer FE — C4. 2FA TOTP | `apps/web` | ⬜ Not started (enroll/challenge; sign-in AAL seam) | TBD |
