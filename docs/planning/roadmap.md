...
| Customer FE — D. Booking & Review | `apps/web` | ✅ Complete — D1 Booking flow → D2 My bookings → D3 Write review all done | see D1–D3 below |
...
| Customer FE — D3. Write review | `apps/web` | ✅ Done on `feat/customer-fe-write-review` (RSC `/account/bookings/[code]/review`: reverse-guard → load booking → PAID eligibility gate (non-PAID → ineligible panel) → `ReviewForm`; RHF+zod (`rating` 1–5 via `@tourism/ui` `Rating` input, `title`≤120, `body` 10–2000) → `createReview` server action → `POST /reviews` → success panel (explicit pending-moderation copy); `mapReviewError` maps `REVIEW_NOT_ELIGIBLE`/`REVIEW_ALREADY_EXISTS`/ownership codes; `BookingDetail` shows a "Write a review" CTA on PAID bookings only (D2 seam closed). **Backend untouched** (review endpoints already enforce ownership + PAID + one-per-booking). EN/VI parity; theme tokens only; TDD on pure logic — 199 web tests; full browser e2e (7 scenarios: seam→form, invalid blocked, valid→pending, duplicate→409, non-PAID gate, signed-out returnTo, VI, console-clean). **Phase D complete.** | [specs/2026-06-14-customer-fe-write-review-design.md](../superpowers/specs/2026-06-14-customer-fe-write-review-design.md), [plans/2026-06-14-customer-fe-write-review.md](../superpowers/plans/2026-06-14-customer-fe-write-review.md) |
